...
const long Kotol_MinCasMedziDvomaZapnutiamiKotla =  2* 60*1000L;
const long Kotol_MinCasBehuKotla =  1* 60*1000L;
const long Kotol_CasDobehCerpadlo =  1* 60*1000L;
...
const long rx433MHz_Timeout = 1 * 60 * 1000L;
...
const long lcd_Timeout = 3 * 1000L;
...
void set_kotol(void)
{
  ...
  if((TempNast > tmp_base) && (KotolStatus == 0)){
    if(time - KotolCasVypnutia > Kotol_MinCasMedziDvomaZapnutiamiKotla){
      set_kotol(1); 
      set_cerpadlo(1); 
      set_rele3(1); 
      ...
      Serial.println("kratky cas medzi dvoma zapnutiami");
    }
  } 
  else {
    if(time - KotolCasZapnutia > Kotol_MinCasBehuKotla){
      ...
      Serial.println("kratky cas zapnutia");
    }
  }
