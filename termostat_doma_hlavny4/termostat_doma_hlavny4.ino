...
void vypis_ostatne(void)
{
  ...
  case 0:
    ...
    if(rx433MHzAgainRury > time){
      vypis_lcd_temp(RuryKotolVystup);
    }
    else{ 
      vypis_lcd_nodata();
    }
  ...
  case 1:
    ...
    lcd.setCursor(14, 3);
    if(rx433MHzAgainRury > time){
      vypis_lcd_temp(RuryKotolSpiatocka);
    }
    else{ 
      vypis_lcd_nodata();
    }
  ...
  case 2:
    ...
    lcd.setCursor(14, 3);
    if(rx433MHzAgainRury > time){
      vypis_lcd_temp(RuryBojlerVystup);
    }
    else{ 
      vypis_lcd_nodata();
    }
...
void vypis_lcd_TempVonku(int input){
  //Serial.println(input);
